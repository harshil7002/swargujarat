...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.currentManageCategory = null;

    async function updateCategoryButtons() {
        const customSongs = await loadSongs();
        const baseCategories = ["Garba", "Gujarati Love", "Gujarati Film Songs", "Gujarati Bhajan", "Mataji Songs", "Krishna Songs", "Wedding Songs"];
        ...
        categories.forEach(cat => {
            ...
            btn.className = 'cat-stat-btn';
            btn.innerHTML = `${cat} <span class="badge">${counts[cat] || 0} +</span>`;
            btn.onclick = () => openManageModal(cat);
            container.appendChild(btn);
        });
    }

    async function openManageModal(categoryName) {
        window.currentManageCategory = categoryName;
        document.getElementById('manage-modal-title').textContent = `Manage: ${categoryName}`;
        document.getElementById('manage-modal-overlay').classList.add('show');
        await renderManageList(categoryName);
    }
    ...
    if(closeManageModalBtn) {
        closeManageModalBtn.addEventListener('click', () => {
            document.getElementById('manage-modal-overlay').classList.remove('show');
            window.currentManageCategory = null;
        });
    }

    async function renderManageList(categoryFilter = null) {
        const listDiv = document.getElementById('manage-song-list');
        if (!listDiv) return;
        
        listDiv.innerHTML = '<p style="color: rgba(255,255,255,0.5); grid-column: 1 / -1;">Loading from cloud...</p>';
        let customSongs = await loadSongs();
        listDiv.innerHTML = '';
        
        let filteredSongs = customSongs.map((song, index) => ({song, index}));
        if(categoryFilter) {
            filteredSongs = filteredSongs.filter(item => (item.song.category || 'All Songs') === categoryFilter);
        }

        if (filteredSongs.length === 0) {
            listDiv.innerHTML = '<p style="color: rgba(255,255,255,0.5); grid-column: 1 / -1;">No songs added in this category yet.</p>';
            updateCategoryButtons();
            return;
        }
        ...
        filteredSongs.forEach(item => {
            ...
            div.innerHTML = `
                <div style="display: flex; gap: 12px; align-items: center;">
                    <img src="${song.cover}" alt="Thumb" style="width: 50px; height: 50px; border-radius: 8px; object-fit: cover;">
                    <div style="flex: 1; min-width: 0;">
                        <h4 style="margin:0; font-size: 14px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${song.title.replace(/"/g, '&quot;')}">${song.title}</h4>
                        <p style="margin:4px 0 0; font-size: 12px; color: rgba(255,255,255,0.7); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${song.artist}</p>
                    </div>
                </div>
                <div style="display: flex; gap: 8px; justify-content: flex-end; margin-top: auto;">
                    <button class="edit-btn" data-index="${index}" style="background: rgba(255, 255, 255, 0.1); color: #fff; border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 8px; padding: 6px 16px; cursor: pointer; font-size: 12px; transition: all 0.2s;">Edit</button>
                    <button class="delete-btn" data-index="${index}" style="background: rgba(255, 82, 82, 0.15); color: #ff5252; border: 1px solid rgba(255, 82, 82, 0.3); border-radius: 8px; padding: 6px 16px; cursor: pointer; font-size: 12px; transition: all 0.2s;">Delete</button>
                </div>
            `;
    ...
    async function deleteCategory(catName) {
        ...
        if (confirmed) {
            ...
            renderManageList(window.currentManageCategory);
        }
    }

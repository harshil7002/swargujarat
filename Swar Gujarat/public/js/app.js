...
class App {
    ...
    async init() {
        ...
        // Group songs by category so they play category by category
        const grouped = {};
        ...
        let sortedPlaylist = [];
        // For each category, shuffle the songs inside it, then add to the main playlist
        Object.keys(grouped).forEach(cat => {
            let catSongs = grouped[cat];
            for (let i = catSongs.length - 1; i > 0; i--) {
                const j = Math.floor(Math.random() * (i + 1));
                [catSongs[i], catSongs[j]] = [catSongs[j], catSongs[i]];
            }
            sortedPlaylist = sortedPlaylist.concat(catSongs);
        });

        // Load sorted playlist to player
        if (sortedPlaylist.length > 0) {
            window.player.loadPlaylist(sortedPlaylist, 0);
        }
        ...
        catButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (window.player) {
                    ...
                    if (window.player.playIcon) window.player.syncUIPlayState(true);
                }
